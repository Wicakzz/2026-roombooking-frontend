...
export default function DetailModal({ isOpen, booking, onClose, onDelete, onEdit }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 animate-in zoom-in duration-200">
        <div className="flex justify-between items-center mb-8">
          <h3 className="text-xl font-extrabold text-slate-800">Info Peminjaman</h3>
        ...
        <div className="space-y-6 mb-8">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ruangan</p>
              <p className="text-lg font-bold text-slate-800">{booking.roomName}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Peminjam</p>
              <p className="text-lg font-bold text-slate-800">{booking.userName}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Waktu Peminjaman</p>
              <p className="text-sm font-bold text-slate-800">{new Date(booking.startTime).toLocaleString('id-ID')}</p>
              <p className="text-xs text-slate-400">s/d {new Date(booking.endTime).toLocaleString('id-ID')}</p>
            </div>
          </div>
